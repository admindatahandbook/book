...
plotallmechs<-function(outputformat) {
      message(paste0("Outputformat = ",outputformat))
   ...
   if ( outputformat == "html" ) {
      ...
      column_spec(6, width="17em", background=(ifelse(summarymechs$range[1:nrow(summarymechs)]=="Highly Restricted",config$colors$aspect_low,ifelse(summarymechs$range[1:nrow(summarymechs)]=="Limited",config$colors$aspect_med,config$colors$aspect_high))))  -> table
      print(table)
   } 
   if ( outputformat == "epub3" ) {
      ...
      kable("html", align="cccccc",
            col.names = c("Data Access Mechanism","Researcher Agency Over Analysis Computer",
                        ...
                        "Access Security",
                        "Range of Analysis Methods Available"),
            caption = "Summary of Access Mechanisms Along the Five Aspects") -> table
      print(table)
   } 

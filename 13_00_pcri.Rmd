...
```{r pcritable1,echo=FALSE,results='asis'}
#pcritable1()
if ( knitr::is_html_output() ) {
pcritable1.csv %>%
   kable(format.args = list(big.mark = ","),
         caption="Number of Distinct private capital firms provided by source of information") %>%
   kable_styling(full_width = F)  %>%
    row_spec(nrow(pcritable1.csv),bold=T) %>%
    footnote(general="As of 2015. Source: @jeng2015.",footnote_as_chunk = T, general_title = "Notes: ")
}

if ( knitr::is_latex_output() ) {
cat("\\input{./tables/pcri/pcritable1.tex}")
}
```

```{r pcrifigure1, echo=FALSE,  fig.align = 'center', fig.cap='Overlap of private capital firms in PCRI database by source of information',eval=knitr::is_html_output() }
knitr::include_graphics('./assets/pcri/pcrifigure1web.png')
```
```{r pcrifigure1-tex,echo=FALSE,results='asis'}
if ( knitr::is_latex_output() ) {
      if ( ! file.exists('./figures/pcrifigure1.png')) {
            file.copy('./assets/pcri/pcrifigure1.png','./figures/pcrifigure1.png')
            }
      cat("\\input{./figures/pcrifigure1.tex}")
}
```
...
```{r pcrifigure2, echo=FALSE, out.width='100%', fig.cap='Table relationship diagram of PCRI database',eval=knitr::is_html_output() }
knitr::include_graphics('./assets/pcri/pcrifigure2web.png')
```
```{r pcrifigure2-tex,echo=FALSE,results='asis'}
if ( knitr::is_latex_output() ) {
      if ( ! file.exists('./figures/pcrifigure2.png')) {
            file.copy('./assets/pcri/pcrifigure2.png','./figures/pcrifigure2.png')
            }
      cat("\\input{./figures/pcrifigure2.tex}")
}
```
...
```{r pcritable2,echo=FALSE,results='asis'}
if ( knitr::is_html_output() ) {
pcritable2.csv %>%
   kable(format.args = list(big.mark = ","),
         caption="Breakdown of state of incorporation of the 622 venture-backed portfolio companies") %>%
   kable_styling(full_width = F)  %>%
    row_spec(nrow(pcritable2.csv),bold=T) 
}

if ( knitr::is_latex_output() ) {
cat("\\input{./tables/pcri/pcritable2.tex}")
}
```
...
`r if ( knitr::is_html_output() ) {knitr::raw_html("Data Agreement (see Appendix B for the draft agreement).")}` 
`r if ( knitr::is_latex_output() ) {knitr::raw_latex("Data Agreement.\\footnote{A sample agreement can be found in the Online Appendix.}")}` 

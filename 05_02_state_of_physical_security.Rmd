...
[Norwegian system, for instance, is running python in the background. Only allow a subset of pandas package

Various remote submission systems that limit what you can do. Look at canadian remote submission system that is essentially a tabulator (variables you can access, SAS code, limited to SAS code, whitelist of admitted commands)

NCHS, similar for their remote submission system [https://www.cdc.gov/rdc/b2accessmod/ACs200.htm - remote access no longer available]

Online tabulators. Can express many things as conditional tables, working through that interface you are limited to tables.

Limited access to different set of data/commands, can access from anywhere, can only do certain things with it
...
https://www.cdc.gov/rdc/b2accessmod/ACs200.htm - remote access no longer available, otherwise available at FSRDCs

#### NLSY - 79 and 97

Data location: On Site at BLS (data provider)
Analysis Computer: Data provider owns, BLS server
Access Computer: data provider owns, ports disabled
Access Room: unknown, presumably high (all materials can be searched)
Analysis Methods: Some restrictions (permission required, but the BLS project coordinator can load software and files onto the account)
